...
. Choose the `.jpi` file from your system or enter a URL to the `.jpi` file under the *Deploy Plugin* section.
...
Assuming a `.jpi` file has been explicitly downloaded by a system
administrator, the administrator can manually place the `.jpi` file in a
specific location on the file system.

Copy the downloaded `.jpi`` file into the `JENKINS_HOME/plugins` directory on
...
[CAUTION]
====
If a plugin `.jpi` file is removed but required by other plugins, the Jenkins
...
`JENKINS_HOME/plugins/PLUGIN_NAME.jpi.pinned` file can also be manually
